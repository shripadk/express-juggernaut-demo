  ...
  publish: function(message){
    ...
    for(var i=0, len = channels.length; i < len; i++) {
      ...
      for(var x=0, len2 = clients.length; x < len2; x++) {
				if(clients[x].session_id != message.except) {
        	clients[x].write(message);
				}
      }

			var allClients = this.find('*').clients;
			for(var x=0, len2 = allClients.length; x < len2; x++) {
				if(allClients[x].session_id != message.except) {
        	allClients[x].write(message);
				}
      }
    }
  },
